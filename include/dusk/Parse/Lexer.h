...
public:
  ...
  Lexer(const llvm::SourceMgr &SM, unsigned BufferID,
        DiagnosticEngine *Engine = nullptr, bool KeepComments = false);
  ...
  void diagnose(diag::DiagID ID = diag::DiagID::lex_unexpected_symbol);
  void diagnose(Token T, diag::DiagID ID);
